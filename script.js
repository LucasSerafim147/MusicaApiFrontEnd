document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderMusic(music) {
      ...
      musicItem.innerHTML = `
          <img src="http://localhost:3000${music.cover}" class="music-cover" alt="Capa ${music.title}">
  ...
  musicForm.addEventListener('submit', (e) => {
      ...
      const url = id ? `http://localhost:3000/api/musics/${id}` : 'http://localhost:3000/api/musics';
  ...
  function deleteMusic(id) {
      if (confirm('Tem certeza que deseja excluir esta música?')) {
          fetch(`http://localhost:3000/api/musics/${id}`, { method: 'DELETE' })
              .then(response => {
